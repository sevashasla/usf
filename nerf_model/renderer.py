...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        if self.use_semantic_uncert:
            semantic_uncertainty_image = semantic_uncertainty_image.view(*prefix, 1)
            semantic_uncert = semantic_uncert.squeeze(-1)
        else:
            semantic_uncertainty_image = None
        ...
        return {
            'depth': depth,
            'image': image,
            'alphas_mean': alphas.reshape(*prefix, -1).mean(dim=(1, 2)).unsqueeze(1), # for loss & choose new k
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.training:
            ...
            sigmas, rgbs, snmtc = self(xyzs, dirs)
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096, **kwargs):
        ...
        if staged and not self.cuda_ray:
            ...
            if self.use_uncert:
                ...
                alphas_mean = torch.zeros((B, 1), device=device)
            else:
                uncertainty_image = None
                alphas_mean = None
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    if self.use_uncert:
                        ...
                        alphas_mean[b:b+1] += results_['alphas_mean'] * (tail - head)
                    if self.use_semantic:
            ...
            results['alphas_mean'] = alphas_mean
