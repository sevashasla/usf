...
if __name__ == '__main__':
    ...
    parser.add_argument('--to_save_path', type=str, help="file to store transforms.json")
    parser.add_argument('--images', type=str, default='images', help="images folder (do not include full path, e.g., just use `images_4`)")
    parser.add_argument('--masks', type=str, default='masks', help="images folder (do not include full path, e.g., just use `images_4`)")
    ...
    masks = [f[len(opt.path):] for f in sorted(glob.glob(os.path.join(opt.path, opt.masks, "*"))) if f.lower().endswith('png') or f.lower().endswith('jpg') or f.lower().endswith('jpeg')]
    ...
    write_json(opt.to_save_path, frames)
